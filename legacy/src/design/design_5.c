...
void		init_y_textbox(t_env *env)
{
	...
	y->change_callback = (t_control_callback)ctrl_y_change;
	y->scroll_callback = (t_scroll_callback)ctrl_y_pos_scroll;
	y->callback_data = env;
	if (env->sel_obj.active_pos == CENTER1)
...
void		init_y_minus_button(t_env *env)
{
	...
	m_btn->click_callback = (t_control_callback)ctrl_y_btn_click;
...
void		init_y_plus_button(t_env *env)
{
	...
	p_btn->click_callback = (t_control_callback)ctrl_y_btn_click;
