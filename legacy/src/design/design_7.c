...
void		init_emission_textbox(t_env *env)
{
	...
	em->change_callback = (t_control_callback)ctrl_emission_change;
...
void		init_p1_textbox(t_env *env)
{
	...
	p->change_callback = (t_control_callback)ctrl_p_change;
	p->scroll_callback = (t_scroll_callback)ctrl_scroll;
	p->callback_data = env;
	ft_bzero(p->text, 32);
