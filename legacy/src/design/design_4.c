...
void		init_x_textbox(t_env *env)
{
	...
	x->change_callback = (t_control_callback)ctrl_x_change;
	x->scroll_callback = (t_scroll_callback)ctrl_x_pos_scroll;
	x->callback_data = env;
	if (env->sel_obj.active_pos == CENTER1)
...
void		init_x_minus_button(t_env *env)
{
	...
	m_btn->click_callback = (t_control_callback)ctrl_x_btn_click;
...
void		init_x_plus_button(t_env *env)
{
	...
	p_btn->click_callback = (t_control_callback)ctrl_x_btn_click;
