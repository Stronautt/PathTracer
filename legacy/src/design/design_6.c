...
void		init_z_textbox(t_env *env)
{
	...
	z->change_callback = (t_control_callback)ctrl_z_change;
	z->scroll_callback = (t_scroll_callback)ctrl_z_pos_scroll;
	z->callback_data = env;
	if (env->sel_obj.active_pos == CENTER1)
...
void		init_z_minus_button(t_env *env)
{
	...
	m_btn->click_callback = (t_control_callback)ctrl_z_btn_click;
...
void		init_z_plus_button(t_env *env)
{
	...
	p_btn->click_callback = (t_control_callback)ctrl_z_btn_click;
